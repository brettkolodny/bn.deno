...
Deno.test("BN.js/Reduction context", () => {
  function testMethod(name, fn) {
    let p = new BN(257);
    let m = fn(p);
    let a = new BN(123).toRed(m);
    let b = new BN(231).toRed(m);

    assertEquals(
      a.redAdd(b).fromRed().toString(10),
      "97",
      "should support add, iadd, sub, isub operations",
    );
    assertEquals(
      a.redSub(b).fromRed().toString(10),
      "149",
      "should support add, iadd, sub, isub operations",
    );
    assertEquals(
      b.redSub(a).fromRed().toString(10),
      "108",
      "should support add, iadd, sub, isub operations",
    );

    assertEquals(
      a.clone().redIAdd(b).fromRed().toString(10),
      "97",
      "should support add, iadd, sub, isub operations",
    );
    assertEquals(
      a.clone().redISub(b).fromRed().toString(10),
      "149",
      "should support add, iadd, sub, isub operations",
    );
    assertEquals(
      b.clone().redISub(a).fromRed().toString(10),
      "108",
      "should support add, iadd, sub, isub operations",
    );
    ...
    a = new BN(123);
    b = new BN(231);
    let c = a.toRed(m).redMul(b.toRed(m)).fromRed();
    assert(
      c.cmp(a.mul(b).mod(p192)) === 0,
    ...
    assertEquals(
      a.toRed(m).redPow(new BN(4)).fromRed().cmp(a.sqr().sqr()),
      0,
      "should support pow and mul operations",
    );
    assertEquals(
      a.toRed(m).redPow(new BN(8)).fromRed().cmp(a.sqr().sqr().sqr()),
      0,
      "should support pow and mul operations",
    );
    assertEquals(
      a.toRed(m).redPow(new BN(9)).fromRed().cmp(a.sqr().sqr().sqr().mul(a)),
      0,
      "should support pow and mul operations",
    );
    assertEquals(
    ...
    assertEquals(
      a.toRed(m).redPow(new BN("deadbeefabbadead", 16)).fromRed().toString(16),
    ...
    var qr = q.redSqrt();
    assertEquals(qr.redSqr().cmp(q), 0, "should sqrtm numbers");
    ...
    b = a.redInvm();
    assertEquals(
      a.redMul(b).fromRed().toString(16),
      "1",
      "should invm numbers",
    );
    ...
    a = new BN("deadbeefabbadead", 16);
    b = new BN("abbadeadbeefdead", 16);
    c = a.mul(b).mod(p);
    ...
    assertEquals(result.toString(), "1", "should pow(base, 0) == 1");

    base = new BN(256).toRed(BN.red("k256"));
    result = base.redShl(1);
    assertEquals(result.toString(), "512", "should shl numbers");
    ...
    assert(func(), "should reduce when converting to red");

    a = new BN(0).toRed(BN.red("k256")).redNeg();
    assertEquals(a.isZero(), true, "redNeg and zero value");
  ...
  let p = BN._prime("k256");

  assertEquals(
    p.ireduce(new BN(0xdead)).toString(16),
    "dead",
  ...
  assertEquals(
    regr2.redInvm().redMul(regr2).fromRed().cmpn(1),
    0,
  ...
  n = new BN("30f28939", 16);
  actual = n.toRed(BN.red("k256")).redISqr().fromRed();
  assertEquals(
    actual.toString(16),
    "95bd93d19520eb1",
  ...
  assertEquals(r.toString(16), expectedR, "should avoid 4.1.0 regresion");
  ...
  red = BN.red(new BN(13));
  const a = new BN(2).toRed(red);
  const b = new BN(7).toRed(red);
  const c = a.redIMul(b);
  assertEquals(a.toNumber(), 1, "imod should change host object");
  assertEquals(c.toNumber(), 1, "imod should change host object");
});
